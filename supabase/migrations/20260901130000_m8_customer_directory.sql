...
begin
  ...
  return jsonb_build_object(
    ...
    'customers',coalesce((select jsonb_agg(jsonb_build_object(
      'userId',u.id,'email',u.email,'name',p.display_name,'signedUpAt',u.created_at,
      'lastActivityAt',(select max(a.created_at) from public.activity_events a where a.workspace_id=wm.workspace_id),
      'plan',s.plan_key,'subscriptionStatus',s.status,'gmailStatus',g.watch_status,'gmailWatchExpiresAt',g.watch_expiration,
      'analysedDeals',coalesce((select sum(uc.value) from public.usage_counters uc where uc.workspace_id=wm.workspace_id and uc.period_end>=current_date),0),
      'failedJobs',(select count(*) from private.ai_analysis_jobs j where j.workspace_id=wm.workspace_id and j.status='failed')+(select count(*) from private.gmail_send_jobs j where j.workspace_id=wm.workspace_id and j.status='failed'),
      'supportGrantActive',exists(select 1 from public.support_access_grants sg where sg.workspace_id=wm.workspace_id and sg.revoked_at is null and sg.expires_at>now())
    ) order by u.created_at desc) from auth.users u join public.workspace_members wm on wm.user_id=u.id left join public.user_profiles p on p.user_id=u.id left join public.subscriptions s on s.workspace_id=wm.workspace_id left join public.gmail_connections g on g.workspace_id=wm.workspace_id),'[]'::jsonb)
  );
end;
$$;
