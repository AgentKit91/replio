...
select throws_ok(format('insert into public.creator_goals(workspace_id,goal_code,priority) values(%L,%L,1)',:'one_workspace_id','other'),'23514','maximum three active goals','three active-goal limit is explicit');
...
reset role;select is(has_schema_privilege('authenticated','private','usage'),false,'authenticated API cannot access the private contribution schema');
select is((select count(*) from private.benchmark_contributions),0::bigint,'raw benchmark store starts empty');
