...
export default async function DealWorkspace({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!deal) notFound();
  const [{ data: threads }, { data: offers }, { data: terms }, { data: deliverables }, { data: notes }, { data: activity }, { data: snapshot }] = await Promise.all([
