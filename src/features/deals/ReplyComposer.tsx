...
export function ReplyComposer({draft}:{draft:Draft}){
  ...
  return <section className="reply-composer" aria-label="Suggested reply editor"><div className="reply-composer-heading"><div><p className="eyebrow">Rep Bureau prepared a reply</p><h3>Edit before sending</h3></div><span className={`save-state save-${status}`}>{status==="saving"?"Saving…":status==="error"?"Save failed":"Saved"}</span></div>
