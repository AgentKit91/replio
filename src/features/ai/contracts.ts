...
export const commercialExtractorOutput = z.object({
  ...
  deliverables: z.array(z.object({ platform: z.string().nullable(), deliverable_type: z.string(), quantity: z.number().int().positive(), evidence: z.array(evidenceSchema) })),
  offers: z.array(z.object({ offered_by: z.enum(["brand","creator"]), amount_minor: z.number().int().nonnegative(), currency: z.string().length(3), offer_type: z.enum(["initial","counter","revised","final"]), evidence: z.array(evidenceSchema) })),
  terms: z.array(z.object({ type: z.string(), state: z.enum(["confirmed","missing","inferred"]), value: z.record(z.string(), z.unknown()), display_value: z.string(), confidence, evidence: z.array(evidenceSchema) })),
  missing_material_terms: z.array(z.string()), confidence,
});
export const pricingOutput = z.object({ ideal_ask_minor: z.number().int().nonnegative(), expected_settlement_minor: z.number().int().nonnegative(), minimum_worthwhile_minor: z.number().int().nonnegative(), currency: z.string().length(3), rationale: z.string(), evidence_categories: z.array(z.string()), missing_material_facts: z.array(z.string()), confidence });
...
export const replyOutput = z.object({ subject: z.string(), body: z.string(), strategy_preserved: z.boolean(), facts_used: z.array(z.string()), confidence });
