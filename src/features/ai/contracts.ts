...
export const evidenceSchema = z.object({ message_id: z.uuid(), locator: z.string().trim().min(1).max(200), excerpt: z.string().trim().min(1).max(280) });
...
export const commercialExtractorOutput = z.object({
  ...
  offers: z.array(z.object({ offered_by: z.enum(["brand","creator"]), amount_minor: z.number().int().nonnegative(), currency: z.string().regex(/^[A-Z]{3}$/), offer_type: z.enum(["initial","counter","revised","final"]), evidence: z.array(evidenceSchema).min(1) })),
  ...
  operational_facts: z.array(z.object({field_key:z.enum(["brand_name","agency_name","contact_name","contact_email","billing_entity","billing_address","accounts_payable_email","purchase_order","invoice_instructions","campaign_start","campaign_end","usage_terms","exclusivity_terms","paid_media_terms"]),display_value:z.string().trim().min(1).max(5000),confidence,evidence:z.array(evidenceSchema).min(1)})).optional(),
  deadlines: z.array(z.object({deadline_type:z.enum(["campaign_start","campaign_end","draft_due","approval_due","publish_due","invoice_due","payment_due","other"]),label:z.string().trim().min(1).max(200),due_at:z.iso.datetime(),confidence,evidence:z.array(evidenceSchema).min(1)})).optional(),
  missing_material_terms: z.array(z.string()), confidence,
});
export const pricingOutput = z.object({ ideal_ask_minor: z.number().int().nonnegative(), expected_settlement_minor: z.number().int().nonnegative(), minimum_worthwhile_minor: z.number().int().nonnegative(), currency: z.string().regex(/^[A-Z]{3}$/), rationale: z.string().trim().min(1), evidence_categories: z.array(z.string()), missing_material_facts: z.array(z.string()), confidence }).superRefine((value,ctx)=>{if(!(value.ideal_ask_minor>=value.expected_settlement_minor&&value.expected_settlement_minor>=value.minimum_worthwhile_minor))ctx.addIssue({code:"custom",message:"Pricing recommendations must descend from ideal to minimum"});});
...
export const strategyOutput = z.object({ primary_objective: z.string().trim().min(1), sequence: z.array(z.string().trim().min(1)).min(1), terms_to_clarify: z.array(z.string()), acceptable_concessions: z.array(z.string()), respectful_challenge: z.string().nullable(), confidence });
export const replyOutput = z.object({ subject: z.string().trim().min(1), body: z.string().trim().min(1), strategy_preserved: z.literal(true), facts_used: z.array(z.string()), confidence });
