...
export async function runDealAnalysis(input:{ admin:SupabaseClient; gateway:AiGateway; snapshotId:string; workspaceId:string; dealId:string; userId:string; messages:Message[] }) {
  ...
  const strategy=workerContracts.strategy_engine.parse(combined.strategy_engine);
  if(!draft){const created=await input.admin.from("reply_drafts").insert({workspace_id:input.workspaceId,deal_id:input.dealId,deal_thread_id:thread.id,subject:input.messages.at(-1)?.subject||reply.subject,body:reply.body,source_snapshot_id:input.snapshotId,provider_thread_id:thread.provider_thread_id,respectful_challenge:strategy.respectful_challenge}).select("id,version,subject,body,source_snapshot_id").single(); if(created.error) throw created.error; draft=created.data;}
