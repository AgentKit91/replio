...
**Overall:** M0–M4 COMPLETE; M5 provider-activation E2E pending; M6–M8 COMPLETE; expanded M9 IN PROGRESS
...
- [x] Expanded Phase 1 source-of-truth amendments merged; Rep Bureau is the user-facing product name while safe internal identifiers remain unchanged.
- [x] Living Deal operations foundation, accessible stage-grouped pipeline and “What actually needs me?” Dashboard implemented.
- [x] Creator-owned operational facts preserve provenance/history and outrank imported or AI values.
- [x] Creator review controls cover deadlines, limited contract status, invoice facts/totals/payment terms and versioned invoice/chase wording without sending externally.
- [x] Deterministic invoice numbering, totals, due dates, immutable private PDF versions and currency-separated financial lifecycle totals implemented.
- [x] Creator-approved invoice and payment-chase delivery uses the provider-neutral draft/queue boundary; managed sends remain disabled pending external activation.
- [x] Explicit `Still waiting`, creator-confirmed `Paid` and confirmation-gated `Written off` decisions preserve payment-status history; payment is never inferred.
- [x] Dedicated managed creator-email foundation uses signed Resend inbound events, conservative forwarding provenance, attachment controls, cross-provider deduplication and fail-closed outbound approval.
...
- [ ] Final isolated backup/restore rehearsal, provider-activated E2Es, legal publication, Google verification and live billing founder decisions.

## Tests last run

8 Sep 2026:

- `pnpm lint`, `pnpm typecheck`, `pnpm test` and `pnpm build` — pass after creator Deal-admin and payment-state controls (113 unit tests across 19 files).
- GitHub Actions app/database jobs — clean for PRs #48 and #49; PR #50 pending at the time of this update.
- Hosted Supabase performance advisor — no actionable warning after all 14 Phase 1 foreign-key indexes and redundant permissive policies were remediated.
- Vercel Production deployment for PR #49 (`986d5bf`) — READY.
...
- The stable `dealcheck-v1` Vercel Preview alias is READY. `/dealcheck` passed 390×844 and 1440×1000 layout checks with no horizontal overflow or browser console warnings/errors. The preview OAuth callback includes the exact active branch alias and resume query; the founder Google account completed the protected sign-in return successfully.
...
- Stripe Preview activation is complete without changing Production: the test secret and webhook signing secret are scoped only to `dealcheck-v1`, the protected Preview destination accepts only `checkout.session.completed`, real £4.99 and £9.99 test Checkouts granted exactly 3 and 10 credits, and replaying the 3-pack event produced no duplicate grant. The hosted account balance is 14 (one lifetime free credit plus 13 purchased credits).
- Owner-gated verification still required before the Definition of Done can be signed off: add a valid card to the existing Vercel AI Gateway team account, then complete the first-user and returning-user result journeys. No further DealCheck implementation or Stripe test-mode work is blocked behind that action.
...
1. Complete automatic validated Deal deltas from selected-message analysis without overwriting creator-owned facts.
2. Complete an isolated production-data restore rehearsal and the provider-activated Gmail/managed-email E2Es at their founder-owned boundaries.
3. Finish the consolidated legal, Google restricted-scope and live-billing activation checklist without guessing founder business decisions.
